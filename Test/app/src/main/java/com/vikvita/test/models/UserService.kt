...
typealias UserListener=(user:List<User>)->Unit
class UserService {
...
init {
    ...
    fun deleteUsers(user:User) {
        val indexForDeleting = users.indexOfFirst { it.id == user.id }
        if (indexForDeleting != -1) {
            users=ArrayList(users)
            users.removeAt(indexForDeleting)
        notifyData()
        }

    }
fun getById(userId:Int):UserDetail{
